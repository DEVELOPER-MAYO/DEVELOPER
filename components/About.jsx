...
export  const About = ({profile, profileAlt,subtitle,title,description,
technologies, quickLinks})=>{
    return(
        <section className="lg:p-20 p-10 bg-zinc-800">
            <header className="flex flex-wrap w-full">
                <div className="relative w-32 h-32 mr-8 mb-8">
                    <Image src={profile} alt={profileAlt} layout="fill" objectFit="cover" 
                    />
                </div>
                 <div>
                    <p className="mb-2 text-sm font-semibold uppercase
                    tracking-wider text-zinc-400">{subtitle}
                    </p>

                  <h1 className="font-serif text-4xl text-zinc-50
                  lg:text-6xl">{title}</h1>
